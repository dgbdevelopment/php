...
class UBerEatsController {
    ...
    private function buildPlatformOrder($order) {
        file_put_contents('LastOrder_log.json', json_encode($order));
        ...
        if ($stmt->execute()) {
            file_put_contents('db_log.txt', date("d/m/Y-H:i:s") . " -> Order inserted successfully." . PHP_EOL, FILE_APPEND);
        } else {
            file_put_contents('db_log.txt',  date("d/m/Y-H:i:s") . " -> Error inserting order: " . json_encode($stmt) . PHP_EOL, FILE_APPEND);
        }
